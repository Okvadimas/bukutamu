...
<form action="" method="post">
	<table class="table" >
		...
		<tr>
			...
			<td></td>
			<td>
				<input  class="form-control" type="text" name="jurusan" placeholder="Asal jurusan" size="30">
			</td>
		</tr>
		<tr>
			<td>program_studi</td>
			<td></td>
			<td>
				<input  class="form-control" type="text" name="program_studi" placeholder="program_studi Pengunjung" size="30">
			</td>
 ...
 if (isset($_POST['tambah'])){
	...
	$program_studi  = htmlspecialchars(strtolower($_POST['program_studi']));
 	...
 	if ($nama == '' || $program_studi =='' || $Tujuan ==''){
 		...
 		$simpan = mysqli_query($konek , "INSERT INTO tamu VALUES (null,'$nama', '$jurusan' , '$program_studi' ,'$tglkunjung', '$Tujuan')
 			");
 		if ($simpan){
 			echo "
 		<script>
 		alert('data berhasil disimpan');
 		alert('SELAMAT DATANG DI PERPUSATAKAAN SEKOLAH');
 		document.location.href = 'tampilawalNON.php';
 		</script>
 		";
 		}else {
